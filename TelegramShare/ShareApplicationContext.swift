...
func applicationContext(accountManager: AccountManager, appGroupPath: String, extensionContext: NSExtensionContext) -> Signal<ShareApplicationContext?, NoError> {
    ...
    return currentAccount(allocateIfNotExists: false, networkArguments: NetworkInitializationArguments(apiId: 2834, languagesCategory: "macos", appVersion: ""), supplementary: true, manager: accountManager, rootPath: appGroupPath, beginWithTestingEnvironment: false, auxiliaryMethods: telegramAccountAuxiliaryMethods) |> mapToSignal { result -> Signal<ShareApplicationContext?, NoError> in
