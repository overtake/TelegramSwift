    ...
    func open() {
        if let content = content?.content, let layout = self.content, let window = kitWindow {
            ...
            if content.embedType == "iframe", content.type != "video", let url = content.embedUrl {
                showModal(with: WebpageModalController(url: url, title: content.websiteName ?? content.title ?? strings().webAppTitle, effectiveSize: content.embedSize?.size, context: layout.context), for: window)
    ...
    override func update(with layout: WPLayout) {
        ...
        if let layout = layout as? WPArticleLayout {
            ...
            if layout.content.image == nil, let file = layout.content.file, let dimension = layout.imageSize {
                ...
                representations.append(TelegramMediaImageRepresentation(dimensions: PixelDimensions(dimension), resource: file.resource, progressiveSizes: [], immediateThumbnailData: nil))
