...
class InstantPageChannelView : View, InstantPageView {
    ...
    init(frameRect: NSRect, channel: TelegramChannel, overlay: Bool, openChannel: @escaping(TelegramChannel)->Void, joinChannel: @escaping(TelegramChannel)->Void) {
        ...
        checkView.sizeToFit()
        joinLayout = TextNode.layoutText(.initialize(string: strings().ivChannelJoin, color: .white, font: .normal(.huge)), nil, 1, .end, NSMakeSize(.greatestFiniteMagnitude, .greatestFiniteMagnitude), nil, false, .left)
    ...
    override func layout() {
        super.layout()
        joinLayout = TextNode.layoutText(.initialize(string: strings().ivChannelJoin, color: overlay ? .white : theme.colors.text, font: .normal(.huge)), nil, 1, .end, NSMakeSize(.greatestFiniteMagnitude, .greatestFiniteMagnitude), nil, false, .left)
