...
class GroupNameRowView : InputDataRowView {
    ...
    override func set(item: TableRowItem, animated: Bool) {
        ...
        if let path = item.photo, let image = NSImage(contentsOf: URL(fileURLWithPath: path)) {
            ...
            let image = TelegramMediaImage(imageId: MediaId(namespace: 0, id: 0), representations: [TelegramMediaImageRepresentation(dimensions: PixelDimensions(image.size), resource: resource, progressiveSizes: [], immediateThumbnailData: nil)], immediateThumbnailData: nil, reference: nil, partialReference: nil, flags: [])
