...
func peerPhotos(account: Account, peerId: PeerId, force: Bool = false) -> Signal<[TelegramPeerPhoto], NoError> {
    ...
    if let photos = photos, photos.time > Date().timeIntervalSince1970, !force {
        ...
        return .single(peerAvatars.with { $0[peerId]?.photos } ?? []) |> then(requestPeerPhotos(postbox: account.postbox, network: account.network, peerId: peerId) |> delay(0.3, queue: .concurrentDefaultQueue()) |> map { photos in
