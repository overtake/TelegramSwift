...
private final class LevelRowView : GeneralRowView {
    ...
    private class LineView : View {
        ...
        func set(rating: TelegramStarRating) {
            ...
            //TODOLANG
            let limitedLayout = TextViewLayout(.initialize(string: "Level \(rating.level)", color: .white, font: .medium(.text)))
            ...
            if let _ = rating.nextLevelStars {
                //TODO
                nextLevel = "Level \(rating.level + 1)"
            } else {
                nextLevel = ""
            }
...
private final class RowItem : GeneralRowItem {
    ...
    init(_ initialSize: NSSize, stableId: AnyHashable, context: AccountContext) {
        ...
        let headerText = NSAttributedString.initialize(string: "This rating reflects your activity on Telegram. What affects it:", color: theme.colors.text, font: .medium(.title))
        ...
        let infoImage_1 = generateTextIcon_AccentBadge(text: "ADDED", bgColor: theme.colors.accent, textColor: theme.colors.underSelectedColor)
        let infoImage_2 = generateTextIcon_AccentBadge(text: "ADDED", bgColor: theme.colors.accent, textColor: theme.colors.underSelectedColor)
        let infoImage_3 = generateTextIcon_AccentBadge(text: "DEDUCTED", bgColor: theme.colors.grayForeground, textColor: theme.colors.underSelectedColor)

        
        //TODOLANG
        options.append(.init(image: NSImage(resource: .iconChannelGift).precomposed(theme.colors.accent), header: .init(.initialize(string: "Gifts from Telegram", color: theme.colors.text, font: .medium(.text))), text: .init(.initialize(string: "100% of the Stars spent on gifts purchased from Telegram.", color: theme.colors.grayText, font: .normal(.text)), cutout: .init(topLeft: NSMakeSize(infoImage_1.backingSize.width + 5, 20))), width: initialSize.width - 40, infoImage: infoImage_1))
        
        options.append(.init(image: NSImage(resource: .iconGiftStars).precomposed(theme.colors.accent), header: .init(.initialize(string: "Gifts and Posts from Users", color: theme.colors.text, font: .medium(.text))), text: .init(.initialize(string: "20% of the Stars spent on resold gifts, paid messages and channel posts.", color: theme.colors.grayText, font: .normal(.text)), cutout: .init(topLeft: NSMakeSize(infoImage_2.backingSize.width + 5, 20))), width: initialSize.width - 40, infoImage: infoImage_2))

        options.append(.init(image: NSImage(resource: .iconStarRefund).precomposed(theme.colors.accent), header: .init(.initialize(string: "Refunds and Conversions", color: theme.colors.text, font: .medium(.text))), text: .init(.initialize(string: "10x of refunded Stars and 85% of bought gifts converted to Stars.", color: theme.colors.grayText, font: .normal(.text)), cutout: .init(topLeft: NSMakeSize(infoImage_3.backingSize.width + 5, 20))), width: initialSize.width - 40, infoImage: infoImage_3))
...
func PeerRatingModalController(context: AccountContext, peer: Peer, rating: TelegramStarRating) -> InputDataModalController {
    ...
    let initialState = State(rating: rating)
    ...
    //TODOLANG
    let controller = InputDataController(dataSignal: signal, title: "Rating")
