                    ...
                    self.statusDisposable.set((combineLatest(updatedStatusSignal, context.account.postbox.mediaBox.resourceData(media.resource)) |> deliverOnResourceQueue |> map {  status, resource -> (MediaResourceStatus, AVGifData?) in
                        ...
                        if let strongSelf = self {
                            ...
                            if isSpoiler {
                                ...
                                current.update(isRevealed: false, updated: mediaUpdated, context: context, imageReference: imageReference, size: size, positionFlags: nil, synchronousLoad: approximateSynchronousValue, isSensitive: false)
