    ...
    func textInset(_ status: Bool) -> CGFloat {
        ...
        case .legacy:
            ...
            if self.highlightVerified, (!self.isLookSavedMessage || self.peerId != self.account.peerId) {
                if let size = PremiumStatusControl.controlSize(self.peer, false, left: false), !status {
                    width += size.width + 2
                }
            }
            #endif
