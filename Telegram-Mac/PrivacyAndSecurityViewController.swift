...
private enum PrivacyAndSecurityEntry: Comparable, Identifiable {
    ...
    var sectionId: Int {
        ...
        case let .securityHeader(sectionId):
            return sectionId
        case let .passcode(sectionId, _s):
            return sectionId
...
//    }
    ...
    override func viewDidLoad() {
        ...
        let arguments = PrivacyAndSecurityControllerArguments(context: context, openBlockedUsers: { [weak self] blockedPeers in
                        ...
                        if let currentInfoDisposable = currentInfoDisposable {
                            let applySetting: Signal<Void, NoError> = privacySettingsPromise.get()
                                ...
                                |> mapToSignal { value, sessions, blockedPeers -> Signal<Void, NoError> in
                                    if let value = value {
                                        privacySettingsPromise.set(.single((AccountPrivacySettings(presence: updated, groupInvitations: value.groupInvitations, voiceCalls: value.voiceCalls, voiceCallsP2P: value.voiceCallsP2P, profilePhoto: value.profilePhoto, forwards: value.forwards, phoneNumber: value.phoneNumber, accountRemovalTimeout: value.accountRemovalTimeout), sessions, blockedPeers)))
                                    }
                                    return .complete()
                            }
                            currentInfoDisposable.set(applySetting.start())
                    ...
                    pushControllerImpl(SelectivePrivacySettingsController(context, kind: .voiceCalls, current: info.voiceCalls, callSettings: info.voiceCallsP2P, updated: { updated, p2pUpdated in
                        if let currentInfoDisposable = currentInfoDisposable {
                            let applySetting: Signal<Void, NoError> = privacySettingsPromise.get()
                                ...
                                |> mapToSignal { value, sessions, blockedPeers -> Signal<Void, NoError> in
                                    if let value = value {
                                        privacySettingsPromise.set(.single((AccountPrivacySettings(presence: value.presence, groupInvitations: value.groupInvitations, voiceCalls: updated, voiceCallsP2P: p2pUpdated ?? value.voiceCallsP2P, profilePhoto: value.profilePhoto, forwards: value.forwards, phoneNumber: value.phoneNumber, accountRemovalTimeout: value.accountRemovalTimeout), sessions, blockedPeers)))
                                    }
                                    return .complete()
                            }
                            currentInfoDisposable.set(applySetting.start())
                    ...
                    pushControllerImpl(SelectivePrivacySettingsController(context, kind: .profilePhoto, current: info.profilePhoto, updated: { updated, _ in
                        if let currentInfoDisposable = currentInfoDisposable {
                            let applySetting: Signal<Void, NoError> = privacySettingsPromise.get()
                                ...
                                |> mapToSignal { value, sessions, blockedPeers -> Signal<Void, NoError> in
                                    if let value = value {
                                        privacySettingsPromise.set(.single((AccountPrivacySettings(presence: value.presence, groupInvitations: value.groupInvitations, voiceCalls: value.voiceCalls, voiceCallsP2P: value.voiceCallsP2P, profilePhoto: updated, forwards: value.forwards, phoneNumber: value.phoneNumber, accountRemovalTimeout: value.accountRemovalTimeout), sessions, blockedPeers)))
                                    }
                                    return .complete()
                            }
                            currentInfoDisposable.set(applySetting.start())
                    ...
                    pushControllerImpl(SelectivePrivacySettingsController(context, kind: .forwards, current: info.forwards, updated: { updated, _ in
                        if let currentInfoDisposable = currentInfoDisposable {
                            let applySetting: Signal<Void, NoError> = privacySettingsPromise.get()
                                ...
                                |> mapToSignal { value, sessions, blockedPeers -> Signal<Void, NoError> in
                                    if let value = value {
                                        privacySettingsPromise.set(.single((AccountPrivacySettings(presence: value.presence, groupInvitations: value.groupInvitations, voiceCalls: value.voiceCalls, voiceCallsP2P: value.voiceCallsP2P, profilePhoto: value.profilePhoto, forwards: updated, phoneNumber: value.phoneNumber, accountRemovalTimeout: value.accountRemovalTimeout), sessions, blockedPeers)))
                                    }
                                    return .complete()
                            }
                            currentInfoDisposable.set(applySetting.start())
            ...
            currentInfoDisposable.set(signal.start(next: { [weak currentInfoDisposable] info, _, _ in
                if let info = info {
                    pushControllerImpl(SelectivePrivacySettingsController(context, kind: .phoneNumber, current: info.phoneNumber, updated: { updated, _ in
                        if let currentInfoDisposable = currentInfoDisposable {
                            let applySetting: Signal<Void, NoError> = privacySettingsPromise.get()
                                |> filter { $0.0 != nil }
                                |> take(1)
                                ...
                                |> mapToSignal { value, sessions, blockedPeers -> Signal<Void, NoError> in
                                    if let value = value {
                                        privacySettingsPromise.set(.single((AccountPrivacySettings(presence: value.presence, groupInvitations: value.groupInvitations, voiceCalls: value.voiceCalls, voiceCallsP2P: value.voiceCallsP2P, profilePhoto: value.profilePhoto, forwards: value.forwards, phoneNumber: updated, accountRemovalTimeout: value.accountRemovalTimeout), sessions, blockedPeers)))
                                    }
                                    return .complete()
                            }
                            currentInfoDisposable.set(applySetting.start())
                        ...
                        let timeoutAction: (Int32) -> Void = { timeout in
                            if let updateAccountTimeoutDisposable = updateAccountTimeoutDisposable {
                                ...
                                let applyTimeout: Signal<Void, NoError> = privacySettingsPromise.get()
                                    ...
                                    |> mapToSignal { value, sessions, blockedPeers -> Signal<Void, NoError> in
                                        if let value = value {
                                            privacySettingsPromise.set(.single((AccountPrivacySettings(presence: value.presence, groupInvitations: value.groupInvitations, voiceCalls: value.voiceCalls, voiceCallsP2P: value.voiceCallsP2P, profilePhoto: value.profilePhoto, forwards: value.forwards, phoneNumber: value.phoneNumber, accountRemovalTimeout: timeout), sessions, blockedPeers)))
                                        }
                                        return .complete()
                                }
...
//                                        }
        ...
        genericView.merge(with: combineLatest(queue: .mainQueue(), statePromise.get(), appearanceSignal, settings, privacySettingsPromise.get(), combineLatest(queue: .mainQueue(), recentPeers(account: context.account), twoStepAccessConfiguration.get(), activeSessions.get(), context.sharedContext.accountManager.accessChallengeData()))
            |> map { state, appearance, proxy, values, additional -> TableUpdateTransition in
                let entries = privacyAndSecurityControllerEntries(state: state, privacySettings: values.0, webSessions: values.1, blockedPeers: values.2, proxy: proxy, recentPeers: additional.0, configuration: additional.1, activeSessions: additional.2, passcodeData: additional.3.data).map{AppearanceWrapperEntry(entry: $0, appearance: appearance)}
