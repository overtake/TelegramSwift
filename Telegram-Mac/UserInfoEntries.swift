...
enum UserInfoEntry: PeerInfoEntry {
    ...
    func item( initialSize:NSSize, arguments:PeerInfoArguments) -> TableRowItem {
        ...
        case let .userName(_, value, viewType):
            ...
            return TextAndLabelItem(initialSize, stableId: stableId.hashValue, label: strings().peerInfoUsername, copyMenuText: strings().textCopyLabelUsername, labelColor: value.count > 1 ? theme.colors.text : theme.colors.accent, text: text, context: arguments.context, viewType: viewType, detectLinks: value.count > 1, isTextSelectable: value.count > 1, _copyToClipboard: {
                arguments.copy(link)
