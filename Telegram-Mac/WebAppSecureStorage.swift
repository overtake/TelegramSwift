...
final class WebAppSecureStorage {
    ...
    static func setValue(context: AccountContext, botId: EnginePeer.Id, key: String, value: String?) -> Signal<Never, WebAppSecureStorage.Error> {
        ...
        |> mapToSignal { uuid, accountPeer in
            ...
            let storageValue = StorageValue(
                ...
                accountName: accountPeer?._asPeer().displayTitle ?? "",
                value: value
            )
    ...
    private static func findRestorableKeys(context: AccountContext, botId: EnginePeer.Id, key: String) -> Signal<[ExistingKey], NoError> {
        ...
        |> mapToSignal { _, accountContexts, _ in
            let signals = accountContexts.map { TelegramEngine(account: $0.1).peers.secureBotStorageUuid() }
    ...
    static func transferAllValues(context: AccountContext, fromUuid: String, botId: EnginePeer.Id) -> Signal<Never, WebAppSecureStorage.Error> {
        ...
        |> mapToSignal { toUuid in
            let query: [String: Any] = [
                ...
                kSecMatchLimit as String: kSecMatchLimitAll,
                kSecReturnAttributes as String: true
                // Don't include kSecReturnData here!
            ]
            ...
            if status == errSecSuccess, let items = result as? [[String: Any]] {
                ...
                for item in items {
                    guard let account = item[kSecAttrAccount as String] as? String,
                          account.hasPrefix(fromPrefix) else {
                        continue
                    }

                    // Step 2: Fetch the value data
                    ...
                    // Add to Keychain (ignore duplicates or handle them if needed)
        ...
        if status == errSecSuccess, let items = result as? [[String: Any]] {
            ...
            for item in items {
                if let account = item[kSecAttrAccount as String] as? String,
                   account.contains("WebBot\(botIdString)U"),
                   account.hasSuffix("Key_\(key)") {
                    
                    // SECOND STEP: Query data for each item
                    let dataQuery: [String: Any] = [
                        kSecClass as String: kSecClassGenericPassword,
                        kSecAttrService as String: "TMASecureStorage",
                        kSecAttrAccount as String: account,
                    ...
                    if dataStatus == errSecSuccess,
                       let valueData = dataResult as? Data,
                       let value = try? JSONDecoder().decode(StorageValue.self, from: valueData) {
                        
                        if let range = account.range(of: "WebBot\(botIdString)U"),
                           let endRange = account.range(of: "Key_\(key)") {
