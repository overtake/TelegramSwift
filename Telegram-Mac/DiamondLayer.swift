...
//            }
    ...
    func playAppearanceAnimation(velocity: CGFloat?, smallAngle: Bool, explode: Bool) {
        if explode {
            ...
            self.time.update(value: 8.0, transition: ContainedViewLayoutTransition.animated(duration: 2.0, curve: .spring))
            ...
            let transition = ContainedViewLayoutTransition.animated(duration: 0.3, curve: .easeOut)
    ...
    func update(context: MetalEngineSubjectContext) {
        ...
        let drawableSize = CGSize(width: self.bounds.width * System.backingScale, height: self.bounds.height * System.backingScale)
...
private func loadCubemap(device: MTLDevice) -> MTLTexture? {
    ...
    for (index, faceName) in faceNames.enumerated() {
        guard let image = NSImage(named: faceName),
              let cgImage = image._cgImage else {
            return nil
        }
