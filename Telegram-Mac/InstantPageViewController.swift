...
class InstantPageViewController: TelegramGenericViewController<ScrollView> {
	...
	func scrollPage(direction: Direction) {
		...
		var newOrigin = genericView.scrollPosition().current.rect.origin
		switch direction {
		case .up:
            // without *2 we will stay at current position
			newOrigin.y -= genericView.clipView.frame.height*2
			if newOrigin.y < 0 {
				newOrigin.y = 0
			}
		case .down:
			newOrigin.y += genericView.clipView.frame.height
