...
enum ReplyThreadMode : Equatable {
    ...
    var originId: MessageId {
        switch self {
        case let .replies(id), let .comments(id), let .topic(id):
            return id
        }
    }
}
...
//        floatingPhotosView.backgroundColor = .random
    ...
    func updateHeader(_ interfaceState:ChatPresentationInterfaceState, _ animated:Bool, _ animateOnlyHeader: Bool = false) {
        ...
        /*
         else if let translateState = interfaceState.translateState, translateState.canTranslate {
            state = .translate(voiceChat, translateState)
        }
         */
        
        var value:ChatHeaderState.Value
        if interfaceState.isSearchMode.0 {
            value = .search(searchInteractions, interfaceState.isSearchMode.1, interfaceState.isSearchMode.2)
...
//        subscriber.putCompletion()
        ...
        if firstTransition, let state = scrollToItem, prevIsLoading {
            ...
            subscriber.putNext(TableUpdateTransition(deleted: removed, inserted: inserted, updated: updated, animated: animated, state: scrollState, grouping: grouping, animateVisibleOnly: true))
            subscriber.putCompletion()
        }
...
//    private var reactionManager:AddReactionManager?
    ...
    private func updateHasPhotos(_ theme: TelegramPresentationTheme) {
        if let peer = self.chatInteraction.peer {
            let peerAccept = peer.isGroup || peer.isChannel || peer.isSupergroup || peer.id == context.peerId || peer.id == repliesPeerId
            ...
            |> map { [weak self] messageIndex, canRead in
                ...
                if canRead {
                    ...
                    if apply {
                        ...
                        if !hasModals(self.context.window) {
                    ...
                    case .voice:
                       ...
                       _ = permission.start(next: { [weak chatInteraction] access in
                            ...
                            if access {
                                ...
                                verifyModal(for: context.window, information: strings().requestAccesErrorHaveNotAccessVoiceMessages, ok: strings().modalOK, cancel: "", option: strings().requestAccesErrorConirmSettings, successHandler: { result in
                                   switch result {
                                   case .thrid:
                    ...
                    case .video:
                        ...
                        _ = permission.start(next: { [weak chatInteraction] access in
                            ...
                            if access {
                                ...
                                verifyModal(for: context.window, information: strings().requestAccesErrorHaveNotAccessVideoMessages, ok: strings().modalOK, cancel: "", option: strings().requestAccesErrorConirmSettings, successHandler: { result in
                                    switch result {
                                    case .thrid:
                                        openSystemSettings(.none)
                                    default:
...
//            self.reactionManager?.clearAndTempLock()
        ...
        let editMessage:(ChatEditState, Date?)->Void = { [weak self] state, atDate in
            ...
            if text.length > chatInteraction.maxInputCharacters {
                if context.isPremium || context.premiumIsBlocked {
                    ...
                    verifyModal(for: context.window, information: strings().chatInputErrorMessageTooLongCountable(text.length - Int(chatInteraction.maxInputCharacters)), ok: strings().alertOK, cancel: "", option: strings().premiumGetPremiumDouble, successHandler: { result in
                        switch result {
                        case .thrid:
        ...
        chatInteraction.unarchive = {
            ...
            let unmuteSignal = context.engine.peers.updatePeerMuteSetting(peerId: peerId, threadId: chatLocation.threadId, muteInterval: nil)
        ...
        chatInteraction.sendPlainText = { [weak self] text in
            if let strongSelf = self, let peer = self?.chatInteraction.presentation.peer, peer.canSendMessage(strongSelf.mode.isThreadMode, threadData: strongSelf.chatInteraction.presentation.threadInfo) {
                ...
                let _ = (Sender.enqueue(input: ChatTextInputState(inputText: text), context: context, peerId: chatInteraction.peerId, replyId: takeReplyId(), sendAsPeerId: presentation.currentSendAsPeerId) |> deliverOnMainQueue).start(completed: scrollAfterSend)
            }
        }
        ...
        chatInteraction.sendLocation = { [weak self] coordinate, venue in
            ...
            self?.chatInteraction.sendMedias([media], ChatTextInputState(), false, nil, false, nil, false)
        }
        ...
        chatInteraction.beginEditingMessage = { [weak self] message in
            ...
            if let editState = self?.chatInteraction.presentation.interfaceState.editState, let window = self?.window, let _ = message  {
                if editState.inputState.inputText != editState.message.text {
                    verifyModal(for: window, information: strings().chatEditCancelText, ok: strings().alertDiscard, successHandler: { _ in
                        process()
                    })
                } else {
                    process()
        ...
        chatInteraction.mentionPressed = { [weak self] in
            let signal = context.engine.messages.earliestUnseenPersonalMentionMessage(peerId: peerId, threadId: chatLocation.threadId)
        ...
        chatInteraction.reactionPressed = { [weak self] in
            if let strongSelf = self {
                let signal = context.engine.messages.earliestUnseenPersonalReactionMessage(peerId: strongSelf.chatInteraction.peerId, threadId: chatLocation.threadId)
                    ...
                    strongSelf.messageActionCallbackDisposable.set((context.engine.messages.requestMessageActionCallback(messageId: messageId, isGame:isGame, password: nil, data: data?.data) |> deliverOnMainQueue).start(next: applyResult, error: { [weak strongSelf] error in
                        ...
                        if let data = data, data.requiresPassword {
                            ...
                            if let errorText = errorText {
                                verifyModal(for: context.window, header: strings().botTransferOwnerErrorTitle, information: errorText, ok: strings().modalOK, cancel: strings().modalCancel, option: install2Fa ? strings().botTransferOwnerErrorEnable2FA : nil, successHandler: { result in
                                    switch result {
                                    case .basic:
                                        break
                                    case .thrid:
        ...
        chatInteraction.scrollToTheFirst = { [weak self] in
            ...
            let historyView = preloadedChatHistoryViewForLocation(scroll, context: context, chatLocation: strongSelf.chatLocation, chatLocationContextHolder: strongSelf.chatLocationContextHolder, tagMask: strongSelf.mode.tagMask, additionalData: [])
        ...
        chatInteraction.revealMedia = { [weak self] messageId in
            self?.updateState { current in
                var current = current
        ...
        chatInteraction.sendMedias = { [weak self] medias, caption, isCollage, additionText, silent, atDate, isSpoiler in
            if let strongSelf = self, let peer = strongSelf.chatInteraction.peer {
                ...
                if canSend {
                    func apply(_ controller: ChatController, atDate: Date?) {
                        let _ = (Sender.enqueue(media: medias, caption: caption, context: context, peerId: controller.chatInteraction.peerId, replyId: takeReplyId(), isCollage: isCollage, additionText: additionText, silent: silent, atDate: atDate, isSpoiler: isSpoiler) |> deliverOnMainQueue).start(completed: scrollAfterSend)
                        controller.nextTransaction.set(handler: afterSentTransition)
                    }
                    switch strongSelf.mode {
                    case .history, .thread:
        ...
        chatInteraction.restartTopic = {
            switch mode {
            case let .thread(data, _):
                _ = context.engine.peers.setForumChannelTopicClosed(id: peerId, threadId: makeMessageThreadId(data.messageId), isClosed: false).start()
            default:
                break
            }
        }
        ...
        chatInteraction.sendCommand = { [weak self] command in
            if let strongSelf = self, let peer = strongSelf.chatInteraction.peer, peer.canSendMessage(strongSelf.mode.isThreadMode, threadData: strongSelf.chatInteraction.presentation.threadInfo) {
                func apply(_ controller: ChatController, atDate: Date?) {
                    ...
                    _ = Sender.enqueue(input: ChatTextInputState(inputText: commandText), context: context, peerId: controller.chatLocation.peerId, replyId: takeReplyId(), atDate: atDate).start(completed: scrollAfterSend)
        ...
        chatInteraction.joinGroupCall = { [weak self] activeCall, joinHash in
            ...
            if let activeCall = currentActiveCall {
                ...
                if peer.groupAccess.canMakeVoiceChat {
                    verifyModal(for: context.window, information: strings().voiceChatChatStartNew, ok: strings().voiceChatChatStartNewOK, successHandler: { _ in
        ...
        chatInteraction.shareContact = { [weak self] peer in
            if let strongSelf = self, let main = strongSelf.chatInteraction.peer {
                ...
                if canSend {
                    _ = Sender.shareContact(context: context, peerId: strongSelf.chatInteraction.peerId, media: media, replyId: takeReplyId()).start(completed: scrollAfterSend)
                    strongSelf.nextTransaction.set(handler: afterSentTransition)
                }
            }
        }
            ...
            if let `self` = self {
                ...
                if let peer = self.chatInteraction.peer as? TelegramChannel {
                    if peer.hasPermission(.pinMessages) || (peer.isChannel && peer.hasPermission(.editAllMessages)) {
                        ...
                        let verify = verifyAlertSignal(for: context.window, header: strings().chatConfirmUnpinHeader, information: strings().chatConfirmUnpin, ok: strings().chatConfirmUnpinOK) |> map { $0 == .basic } |> filter { $0 }
                        
                        self.updatePinnedDisposable.set(((dismiss ? verify : Signal<Bool, NoError>.single(true)) |> mapToSignal { _ in return
                    ...
                    if dismiss {
                        verifyModal(for: context.window, header: strings().chatConfirmUnpinHeader, information: strings().chatConfirmUnpin, ok: strings().chatConfirmUnpinOK, successHandler: { [weak self] _ in
        ...
        chatInteraction.unpinAllMessages = { [weak self, unowned context] in
            ...
            if canManagePin {
                ...
                verifyModal(for: context.window, information: strings().chatUnpinAllMessagesConfirmationCountable(count), ok: strings().chatConfirmUnpinOK, cancel: strings().modalCancel, successHandler: { [weak self] _ in
        ...
        chatInteraction.closeAfterPeek = { [weak self] peek in
            ...
            let showConfirm:()->Void = {
                verifyModal(for: context.window, header: strings().privateChannelPeekHeader, information: strings().privateChannelPeekText, ok: strings().privateChannelPeekOK, cancel: strings().privateChannelPeekCancel, successHandler: { _ in
        ...
        let getPinned:()-> Signal<ChatPinnedMessage?, NoError> = { [weak self] in
            ...
            let replyHistoryFirst: Signal<ChatHistoryViewUpdate, NoError> = preloadedChatHistoryViewForLocation(.Initial(count: 6), context: self.context, chatLocation: self.chatLocation, chatLocationContextHolder: self.chatLocationContextHolder, tagMask: MessageTags.pinned, additionalData: [])
        ...
        let threadInfo: Signal<MessageHistoryThreadData?, NoError>
        switch chatLocation {
        case .peer:
            threadInfo = .single(nil)
        case .thread(let data):
            let key: PostboxViewKey = .messageHistoryThreadInfo(peerId: peerId, threadId: makeMessageThreadId(data.messageId))
            threadInfo = context.account.postbox.combinedView(keys: [key]) |> map { views in
        ...
        case .history, .thread:
            ...
            self.chatUnreadMentionCountDisposable.set(combineLatest(queue: .mainQueue(), context.account.viewTracker.unseenPersonalMessagesAndReactionCount(peerId: peerId, threadId: chatLocation.threadId), unreadCount).start(next: { [weak self] count, unreadCount in
       ...
       // var beginHistoryTime:CFAbsoluteTime?
        ...
        self.setLocation(.init(content: location, id: id))
        ...
        editButton.contextMenu = { [weak self] in
            ...
            case .scheduled:
                items.append(ContextMenuItem(strings().chatContextClearScheduled, handler: {
                    verifyModal(for: context.window, header: strings().chatContextClearScheduledConfirmHeader, information: strings().chatContextClearScheduledConfirmInfo, ok: strings().chatContextClearScheduledConfirmOK, successHandler: { _ in
        ...
        chatInteraction.update(animated: false, {$0.withToggledSidebarEnabled(FastSettings.sidebarEnabled).withToggledSidebarShown(FastSettings.sidebarShown)})
         ...
         |> deliverOnMainQueue).start(next: { [weak self] reason in
            if let strongSelf = self {
                ...
                verifyModal(for: context.window, information: text, cancel: "", option: strings().genericErrorMoreInfo, successHandler: { [weak strongSelf] confirm in
                    guard let strongSelf = strongSelf else {return}
...
//                self.reactionManager?.update()
       ...
       self.sentMessageEventsDisposable.set((context.account.pendingMessageManager.deliveredMessageEvents(peerId: self.chatLocation.peerId) |> deliverOn(Queue.concurrentDefaultQueue())).start(next: { result in
           
           if FastSettings.inAppSounds, !result.silent {
        ...
        suggestionsDisposable.set(suggestions.start(next: { suggestions in
            for suggestion in suggestions {
                ...
                case .convertToGigagroup:
                    verifyModal(for: context.window, header: strings().broadcastGroupsLimitAlertTitle, information: strings().broadcastGroupsLimitAlertText(Formatter.withSeparator.string(from: NSNumber(value: context.limitConfiguration.maxSupergroupMemberCount))!), ok: strings().broadcastGroupsLimitAlertLearnMore, successHandler: { _ in
    ...
    func notify(with value: Any, oldValue: Any, animated:Bool) {
        notify(with: value, oldValue: oldValue, animated: animated && self.didSetReady, force: false)
    }
