...
//            self.reactionManager?.clearAndTempLock()
        ...
        chatInteraction.sendInlineResult = { [weak self] (results,result) in
            if let strongSelf = self {
                func apply(_ controller: ChatController, atDate: Int32?) {
                    ...
                    let value = context.engine.messages.enqueueOutgoingMessageWithChatContextResult(to: chatInteraction.peerId, botId: results.botId, result: result, replyToMessageId: takeReplyId())
                    if value {
                        controller.nextTransaction.set(handler: afterSentTransition)
                    }
        ...
        case .history:
            ...
            case let .peer(peerId):
                let replyHistory: Signal<ChatHistoryViewUpdate, NoError> = preloadedChatHistoryViewForLocation(.Initial(count: 100), context: self.context, chatLocation: .peer(peerId), chatLocationContextHolder: self.chatLocationContextHolder, tagMask: MessageTags.pinned, additionalData: [])
    ...
    public init(context: AccountContext, chatLocation:ChatLocation, mode: ChatMode = .history, messageId:MessageId? = nil, initialAction: ChatInitialAction? = nil, chatLocationContextHolder: Atomic<ChatLocationContextHolder?>? = nil) {
        ...
        _ = initialLocation.start(next: { [weak self] view in
            if let strongSelf = self {
                ...
                let location:ChatHistoryLocation
                if let messageId = messageId {
                    location = .InitialSearch(location: .id(messageId), count: max(count, 50))
                } else {
                    location = .Initial(count: count)
                }
