        ...
        case let .unique(gift):
            for attr in gift.attributes {
                ...
                case .model(let name, _, let rarity):
                    rows.append(.init(left: .init(.initialize(string: strings().giftUniqueModel, color: theme.colors.text, font: .normal(.text))), right: .init(name: .init(.initialize(string: name)), badge: .init(text: "\((Double(rarity) / 10).string)%", callback: {}))))
                case .pattern(let name, _, let rarity):
                    rows.append(.init(left: .init(.initialize(string: strings().giftUniqueSymbol, color: theme.colors.text, font: .normal(.text))), right: .init(name: .init(.initialize(string: name)), badge: .init(text: "\((Double(rarity) / 10).string)%", callback: {}))))
                case .backdrop(let name, _, _, _, _, let rarity):
                    rows.append(.init(left: .init(.initialize(string: strings().giftUniqueBackdrop, color: theme.colors.text, font: .normal(.text))), right: .init(name: .init(.initialize(string: name)), badge: .init(text: "\((Double(rarity) / 10).string)%", callback: {}))))
                default:
                    break
                }
            }
