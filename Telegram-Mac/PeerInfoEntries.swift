...
func peerInfoEntries(view: PeerView, threadData: MessageHistoryThreadData?, arguments: PeerInfoArguments, inputActivities: [PeerId: PeerInputActivity], channelMembers: [RenderedChannelParticipant], mediaTabsData: PeerMediaTabsData, inviteLinksCount: Int32, joinRequestsCount: Int32, availableReactions: AvailableReactions?, source: PeerInfoController.Source, story: EngineStorySubscriptions.Item?) -> [PeerInfoEntry] {
    if let threadData = threadData {
        ...
        return userInfoEntries(view: view, arguments: arguments, mediaTabsData: mediaTabsData, source: source, story: story)
    } else if let channel = peerViewMainPeer(view) as? TelegramChannel {
        switch channel.info {
        case .broadcast:
            return channelInfoEntries(view: view, arguments: arguments, mediaTabsData: mediaTabsData, inviteLinksCount: inviteLinksCount, joinRequestsCount: joinRequestsCount, availableReactions: availableReactions)
        case .group:
