...
func ChannelDiscussionSetupController(context: AccountContext, peer: Peer)-> InputDataController {
        ...
        actionsDisposable.add(showModalProgress(signal: signal |> deliverOnMainQueue, for: context.window).start(next: { result in
            ...
            if let error = upgradeError {
                ...
                case .groupHistoryIsCurrentlyPrivate:
                    verifyModal(for: context.window, information: strings().discussionControllerErrorPreHistory, ok: strings().discussionControllerErrorOK, successHandler: { _ in
    ...
    let arguments = DiscussionArguments(context: context, createGroup: {
        ...
        if associated.isChannel {
            verifyModal(for: context.window, information: strings().discussionControllerConfrimUnlinkChannel, successHandler: { _ in
                setup(associated.id, nil)
            })
        } else {
            verifyModal(for: context.window, information: strings().discussionControllerConfrimUnlinkGroup, successHandler: { _ in
                setup(peer.id, nil)
            })
        }
    })
