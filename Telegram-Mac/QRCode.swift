...
private func floorToContextPixels(_ value: CGFloat, scale: CGFloat = System.backingScale) -> CGFloat {
    return floor(value * scale) / scale
}

private func roundToContextPixels(_ value: CGFloat, scale: CGFloat = System.backingScale) -> CGFloat {
...
public func qrCode(string: String, color: NSColor, backgroundColor: NSColor? = nil, icon: QrCodeIcon, ecl: String = "M") -> Signal<(Int, (TransformImageArguments) -> DrawingContext?), NoError> {
...
//                context.translateBy(x: -CGFloat(size) / 2.0, y: -CGFloat(size) / 2.0)
                ...
                let ciContext = CIContext(cgContext: context, options: [CIContextOption.useSoftwareRenderer : NSNumber(value: true)])
        ...
        |> map { data, size, bytesPerRow in
            return (size, { arguments in
                ...
                let tmpContext = DrawingContext(size: CGSize(width: squareSize.width * 4.0, height: squareSize.height), scale: arguments.scale, clear: true)
...
//                    var path = NSBezierPath(roundedRect: CGRect(origin: CGPoint(), size: squareSize), byRoundingCorners: .allCorners, cornerRadii: CGSize(width: outerRadius, height: outerRadius))
                    c.addPath(CGPath(roundedRect: CGRect(origin: CGPoint(), size: squareSize), cornerWidth: outerRadius, cornerHeight: outerRadius, transform: nil))
                    c.fillPath()
...
//                    path = NSBezierPath(roundedRect: CGRect(origin: CGPoint(x: squareSize.width, y: 0.0), size: squareSize), byRoundingCorners: .allCorners, cornerRadii: CGSize(width: innerRadius, height: innerRadius))
                    c.addPath(CGPath(roundedRect: CGRect(origin: CGPoint(x: squareSize.width, y: 0.0), size: squareSize), cornerWidth: innerRadius, cornerHeight: innerRadius, transform: nil))
                    c.fillPath()
                ...
                func drawAt(x: Int, y: Int, fill: Bool, corners: NSRectCorner) {
                ...
                context.withContext { c in
                    ...
                    func drawMarker(x: CGFloat, y: CGFloat) {
                        //var path = NSBezierPath(roundedRect: CGRect(x: x + squareSize.width / 2.0, y: y + squareSize.width / 2.0, width: markerSide, height: markerSide), cornerRadius: markerSide / 3.5)
                        c.addPath(CGPath(roundedRect: CGRect(x: x + squareSize.width / 2.0, y: y + squareSize.width / 2.0, width: markerSide, height: markerSide), cornerWidth: markerSide / 3.5, cornerHeight: markerSide / 3.5, transform: nil))
                        c.strokePath()
...
//                        path = NSBezierPath(roundedRect: CGRect(x: x + squareSize.width * 2.0, y: y + squareSize.height * 2.0, width: dotSide, height: dotSide), cornerRadius: dotSide / 3.5)
                    ...
                    drawMarker(x: CGFloat(size - 2) * squareSize.width - markerSide, y: CGFloat(size - 2) * squareSize.height - markerSide)
                    ...
                    case let .custom(image):
                        if let image = image {
                            let fittedSize = image.size.aspectFitted(NSMakeSize(clipRect.width - 10, clipRect.height - 10))
