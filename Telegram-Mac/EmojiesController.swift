    ...
    if let search = state.search {
        ...
        if !search.isEmpty {
            ...
            if !animatedEmoji.isEmpty {
                ...
                let chunks = animatedEmoji.chunks(32)
                var string: String = "a"
                for chunk in chunks {
...
final class EmojiesController : TelegramGenericViewController<AnimatedEmojiesView>, TableViewDelegate {
    ...
    init(_ context: AccountContext, mode: Mode = .emoji, selectedItems: [EmojiesSectionRowItem.SelectedItem] = [], presentation: TelegramPresentationTheme? = nil) {
        self.mode = mode
        self.presentation = presentation
