...
private func listener(_ event: AUVoiceIOSpeechActivityEvent) -> Void {
    switch AUVoiceIOSpeechActivityEvent(rawValue: event.rawValue) {
    case .hasStarted:
        MutedSpeechMonitor.sharedInstance?.onStatusChanged?(true)
    case .hasEnded:
        MutedSpeechMonitor.sharedInstance?.onStatusChanged?(false)
    default:
        break
    }
}

final class MutedSpeechMonitor {
    private var audioUnit: AudioUnit?
    private let queue = DispatchQueue(label: "muted-speech-monitor")

    fileprivate var onStatusChanged: ((Bool) -> Void)?

    // MARK: - Shared static listener
    fileprivate static var sharedInstance: MutedSpeechMonitor?

    var eventListener: AUVoiceIOMutedSpeechActivityEventListener?


    // MARK: - Public API

    func start(onStatusChanged: @escaping (Bool) -> Void) {
    ...
    private func setupAudioUnit() {
        ...
        guard let component = AudioComponentFindNext(nil, &desc) else {
            print("MutedSpeechMonitor: Failed to find AudioComponent")
            return
        }

        var maybeUnit: AudioUnit?
        guard AudioComponentInstanceNew(component, &maybeUnit) == noErr, let unit = maybeUnit else {
            print("MutedSpeechMonitor: Failed to create AudioUnit")
            return
        }
        ...
        guard AudioUnitSetProperty(unit,
                                   kAudioOutputUnitProperty_EnableIO,
                                   kAudioUnitScope_Input,
                                   1,
                                   &enableIO,
                                   UInt32(MemoryLayout<UInt32>.size)) == noErr else {
            print("MutedSpeechMonitor: Failed to enable IO")
            return
        }

        eventListener = { event in
                   // Your event handling code here
            print("Speech activity detected: \(event)")
            // Do whatever you need with the event
        }
               
        var listenerPtr = eventListener
        
        withUnsafePointer(to: listenerPtr) { listenerPtr in
                    // Set the property on the audio unit
            let propertyStatus = AudioUnitSetProperty(
                unit,
                kAUVoiceIOProperty_MutedSpeechActivityEventListener,
                kAudioUnitScope_Global,
                1,  // Input scope element
                listenerPtr,
                UInt32(MemoryLayout<AUVoiceIOMutedSpeechActivityEventListener>.size)
            )
            
            if propertyStatus != noErr {
                print("SpeechActivityMonitor: Failed to set event listener, error: \(propertyStatus)")
                self.eventListener = nil
                return
            }
            
            print("SpeechActivityMonitor: Successfully started monitoring")
        }


        guard AudioUnitInitialize(unit) == noErr else {
            print("MutedSpeechMonitor: Failed to initialize AudioUnit")
            return
        }

        guard AudioOutputUnitStart(unit) == noErr else {
            print("MutedSpeechMonitor: Failed to start AudioUnit")
            return
        }

        print("MutedSpeechMonitor: Started")
    }
    
    func setMuted(_ muted: Bool) {
        queue.async {
            guard let unit = self.audioUnit else { return }

            var mute: UInt32 = muted ? 1 : 0
            let result = AudioUnitSetProperty(unit,
            ...
            if result != noErr {
                print("🔇 Failed to set mute output: \(result)")
            } else {
                print("🔇 Mic uplink is now \(muted ? "muted" : "unmuted")")
            }
        }
    }
}
