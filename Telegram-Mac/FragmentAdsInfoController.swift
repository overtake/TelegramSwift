//
//  FragmentAdsInfoController.swift
//  Telegram
//
...
private final class RowItem : GeneralRowItem {
    ...
    init(_ initialSize: NSSize, stableId: AnyHashable, context: AccountContext, dismiss:@escaping()->Void) {
        ...
        options.append(.init(image: NSImage(resource: .iconFragmentSplitRevenue).precomposed(theme.colors.accent), header: .init(.initialize(string: strings().fragmentAdsInfoOption2Title, color: theme.colors.text, font: .medium(.text))), text: .init(.initialize(string: strings().fragmentAdsInfoOption2Info, color: theme.colors.grayText, font: .normal(.text))), width: initialSize.width - 40))

        
        options.append(.init(image: NSImage(resource: .iconFragmentNoAds).precomposed(theme.colors.accent), header: .init(.initialize(string:  strings().fragmentAdsInfoOption3Title, color: theme.colors.text, font: .medium(.text))), text: .init(.initialize(string:  strings().fragmentAdsInfoOption3Info, color: theme.colors.grayText, font: .normal(.text))), width: initialSize.width - 40))
