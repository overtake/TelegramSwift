        ...
        if let item = item {
           ...
           // super.draw(layer, in: ctx)
            ...
            if let channelViews = item.channelViews {
                ...
                viewsOffset += channelViews.0.size.width + icon.size.width + 4
            }
            if let replyCount = item.replyCount {
                let icon = item.presentation.chat.repliesCountIcon(item)
                ctx.draw(icon, in: NSMakeRect(replyCount.0.size.width + 2 + item.stateOverlayAdditionCorner + viewsOffset, item.isBubbled ? (item.isStateOverlayLayout ? 1 : 0) : 2, icon.backingSize.width, icon.backingSize.height))
