//
//  VoiceChatMicrophoneView.swift
//  Telegram
//
//  Created by Mikhail Filimonov on 15.12.2020.
...
private final class VoiceChatMicrophoneNodeDrawingState: NSObject {
    ...
    init(color: NSColor, transition: CGFloat, reverse: Bool) {
        self.color = color
...
final class VoiceChatMicrophoneView: View {
    class State: Equatable {
        let muted: Bool
        let raisedHand: Bool
        let canRaiseHand: Bool
        let allowToSpeak: Bool
        let color: NSColor

        init(muted: Bool, raisedHand: Bool, canRaiseHand: Bool, allowToSpeak: Bool, color: NSColor) {
            self.muted = muted
            self.color = color
            self.raisedHand = raisedHand
            self.canRaiseHand = canRaiseHand
            self.allowToSpeak = allowToSpeak
        }
        ...
        static func ==(lhs: State, rhs: State) -> Bool {
            ...
            if lhs.canRaiseHand != rhs.canRaiseHand {
                return false
            }
            if lhs.allowToSpeak != rhs.allowToSpeak {
                return false
            }
            return true
        }
    ...
    private var state: State = State(muted: false, raisedHand: false, canRaiseHand: false, allowToSpeak: false, color: .black)
    ...
    fileprivate var drawParameters: VoiceChatMicrophoneNodeDrawingState {
    ...
    override func draw(_ layer: CALayer, in context: CGContext) {
        ...
        let parameters = self.drawParameters

       


        if self.state.canRaiseHand && !state.allowToSpeak {
            context.draw(GroupCallTheme.big_raised_hand, in: focus(GroupCallTheme.big_raised_hand.backingSize))
        } else {
            ...
            if parameters.transition > 0.0 {
                ...
                let length: CGFloat
                if bounds.width > 30.0 {
                    origin = CGPoint(x: 9.0, y: 10.0)
                    length = 17.0
                } else {
                    origin = CGPoint(x: 5.0, y: 4.0)
                    length = 15.0
                }
