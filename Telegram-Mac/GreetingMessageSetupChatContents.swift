//
//  GreetingMessageSetupChatContents.swift
//  Telegram
//
...
final class GreetingMessageSetupChatContents: ChatCustomContentsProtocol {
    private final class Impl {
        ...
        func enqueueMessages(messages: [EnqueueMessage]) -> [MessageId?] {
            var ids: [MessageId?] = []
            for message in messages {
                ...
                case let .message(text, attributes, _, mediaReference, _, _, _, localGroupingKey, correlationId, _):
                    ...
                    let mappedMessage = Message(
                        ...
                        customTags: [],
                        forwardInfo: nil,
                        author: context.myPeer,
                        text: text,
                        attributes: attributes,
                        media: media,
                        peers: context.myPeer.flatMap { SimpleDictionary([$0.id : $0]) } ?? SimpleDictionary(),
    ...
    func enqueueMessages(messages: [EnqueueMessage]) -> Signal<[MessageId?],NoError> {
        return self.impl.signalWith { impl, subscriber in
            let ids = impl.enqueueMessages(messages: messages)
