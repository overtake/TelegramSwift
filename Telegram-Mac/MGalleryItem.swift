...
func <(lhs: GalleryEntry, rhs: GalleryEntry) -> Bool {
    ...
    case let .photo(lhsIndex, _, _, _, _, _, _, _):
        if  case let .photo(rhsIndex, _, _, _, _, _, _, _) = rhs {
            return lhsIndex < rhsIndex
        } else {
            return false
        }
...
func ==(lhs: GalleryEntry, rhs: GalleryEntry) -> Bool {
    ...
    case let .photo(lhsIndex, lhsStableId, lhsPhoto, lhsReference, lhsPeer, _, lhsDate, lhsCaption):
        if  case let .photo(rhsIndex, rhsStableId, rhsPhoto, rhsReference, rhsPeer, _, rhsDate, rhsCaption) = rhs {
            return lhsIndex == rhsIndex && lhsStableId == rhsStableId && lhsPhoto.isEqual(to: rhsPhoto) && lhsReference == rhsReference && lhsPeer.isEqual(rhsPeer) && lhsDate == rhsDate && lhsCaption == rhsCaption
        } else {
            return false
        }
    case let  .instantMedia(lhsMedia, _):
...
enum GalleryEntry : Comparable, Identifiable {
    ...
    case photo(index:Int, stableId:AnyHashable, photo:TelegramMediaImage, reference: TelegramMediaImageReference?, peer: Peer, message: Message?, date: TimeInterval, caption: String?)
    ...
    var stableId: AnyHashable {
        ...
        case let .photo(_, stableId, _, _, _, _, _, _):
            return stableId
    ...
    var interfaceState:(PeerId, TimeInterval)? {
        ...
        case let .photo(_, _, _, _, peer, _, date, _):
            return (peer.id, date)
        default:
            return nil
        }
    ...
    var peer: Peer? {
        switch self {
        case let .photo(_, _, _, _, peer, _, _, _):
            return peer
        default:
            return nil
        }
    ...
    func peerPhotoResource() -> MediaResourceReference {
        switch self {
        case let .photo(_, _, image, _, peer, message, _, _):
    ...
    var identifier: String {
        ...
        case .photo(_, let stableId, _, _, _, _, _, _):
            return "\(stableId)"
        case .instantMedia:
            return "\(stableId)"
        case let .secureIdDocument(document, _):
    ...
    var photoReference:TelegramMediaImageReference? {
        ...
        case .message:
            return nil
        case let .photo(_, _, _, reference, _, _, _, _):
            return reference
        default:
            return nil
        }
...
class MGalleryItem: NSObject, Comparable, Identifiable {
    ...
    init(_ context: AccountContext, _ entry:GalleryEntry, _ pagerSize:NSSize) {
        ...
        if let message = entry.message, !message.text.isEmpty, !(message.effectiveMedia is TelegramMediaWebpage) {
            ...
            switch entry {
            case let .photo(_, _, _, _, _, _, _, caption):
