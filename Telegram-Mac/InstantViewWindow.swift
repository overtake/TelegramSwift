...
class InstantViewController : TelegramGenericViewController<InstantWindowContentView> {
    ...
    init( page: InstantPageViewController, context: AccountContext) {
        ...
        __window = Window(contentRect: center, styleMask: [.closable, .resizable, .miniaturizable, .fullSizeContentView, .titled, .unifiedTitleAndToolbar, .texturedBackground], backing: .buffered, defer: true)
        navigation = MajorNavigationController(ViewController.self, page, __window)
        navigation.alwaysAnimate = true

        super.init(context)
        __window.isMovableByWindowBackground = false
        __window.name = "Telegram.InstantViewWindow"
        __window.initSaver()
        __window.contentView = genericView
        __window.titleVisibility = .hidden
        navigation._frameRect = NSMakeRect(0, 0, genericView.frame.width, genericView.frame.height - barHeight)
        
        __window.titlebarAppearsTransparent = true
        __window.minSize = NSMakeSize(500, 600)
        ...
        __window.closeInterceptor = { [weak self] in
            if let window = self?.__window, !window.styleMask.contains(.fullScreen) {
                ...
                self?.__window.toggleFullScreen(nil)
                delay(0.8, closure: {
                    self?.__window.orderOut(self)
                    instantController = nil
                })
            }
            return true
        ...
        __window.set(handler: { [weak page] _ in
        ...
        __window.set(handler: closeKeyboardHandler, with: self, for: .Escape)
		if FastSettings.instantViewScrollBySpace {
			...
			__window.set(handler: spaceScrollDownKeyboardHandler, with: self, for: .Space, priority: .low)
			...
			__window.set(handler: spaceScrollUpKeyboardHandler, with: self, for: .Space, priority: .medium, modifierFlags: [.shift])
		} else {
			__window.set(handler: closeKeyboardHandler, with: self, for: .Space)
		}
    ...
    private var titleView: NSView? {
        if let windowView = __window.contentView?.superview {
    ...
    private func share() {
        if let currentPageUrl = currentPageUrl {
            showModal(with: ShareModalController(ShareLinkObject(context, link: currentPageUrl)), for: __window)
        }
    }
    
    override func navigationWillChangeController() {
        genericView.updateTitle(navigation.controller.defaultBarTitle, animated: true)
        __window.title = navigation.controller.defaultBarTitle
    ...
    override func updateLocalizationAndTheme(theme: PresentationTheme) {
        super.updateLocalizationAndTheme(theme: theme)
        let theme = (theme as! TelegramPresentationTheme)
        __window.appearance = theme.appearance
        __window.backgroundColor = theme.colors.grayBackground
    }
    ...
    func show() {
        ...
        self.ready.set(navigation.controller.ready.get() |> take(1) |> filter {$0} |> deliverOnMainQueue |> map { [weak self] value in
          ...
          //  let fromRect = NSMakeRect(toRect.minX + (toRect.width - 100) / 2, toRect.minY + (toRect.height - 100) / 2, 100, 100)
            ...
            self.__window.makeKeyAndOrderFront(nil)
...
func showInstantPage(_ page: InstantPageViewController) {
    if let instantController = instantController {
        if page.webPage.webpageId != (instantController.navigationController?.controller as? InstantPageViewController)?.webPage.webpageId {
            ...
            instantController.__window.orderFront(nil)
            instantController.__window.deminiaturize(nil)
        }
    } else {
