    ...
    override func layout() {
        ...
        if let item = item as? ShortPeerRowItem {
            ...
            case .legacy:
                ...
                separator.frame = NSMakeRect(item.textInset(false), containerView.frame.height - .borderSize, containerView.frame.width - (item.drawSeparatorIgnoringInset ? 0 : item.inset.right) - item.textInset(false), .borderSize)
