...
enum GroupParticipantFlags : td::uint32 {
  AddUsers = 1 << 0,
  RemoveUsers = 1 << 1,
};
...
inline td::StringBuilder &operator<<(td::StringBuilder &sb, const ChangeSetGroupState &state) {
  return sb << "SetGroupState{praticipants_n=" << state.group_state->participants.size() << "}";
}
...
inline td::StringBuilder &operator<<(td::StringBuilder &sb, const ChangeSetSharedKey &shared_key) {
  return sb << "SetSharedKey";
}

struct Change {
  std::variant<ChangeSetValue, ChangeSetGroupState, ChangeSetSharedKey> value;
...
struct State {
  ...
  // TODO apply
  td::Status set_value(td::Slice key, td::Slice value);
  td::Status set_group_state(GroupStateRef group_state, const GroupParticipant &participant);
  td::Status clear_shared_key();
  td::Status set_shared_key(GroupSharedKeyRef shared_key);
  td::Status set_value_fast(KeyValueHash key_value_hash);
  td::Status apply_change(const Change &change_outer, const GroupParticipant &participant, bool full_apply);
