...
GroupStateRef BlockBuilder::make_group_state(std::vector<GroupParticipant> users) {
  return std::make_shared<GroupState>(GroupState{std::move(users)});
}
...
BlockBuilder &BlockBuilder::with_group_state(const std::vector<GroupParticipant> &users, bool in_changes,
                                             bool in_proof) {
  auto state = make_group_state(users);
  if (in_changes) {
...
void BlockchainTester::add_proof(const std::vector<Change> &changes) {
  ...
  for (auto &change_v : changes) {
    std::visit(td::overloaded([&keys](const ChangeSetValue &change) { keys.push_back(change.key); },
