...
//
// Is it too complicated? Is it neccessary to encrypt
...
struct BaselineBlockchainState {
  ...
  void apply_changes(const std::vector<Change> &changes) {
    for (const auto &change_v : changes) {
      std::visit(td::overloaded([&](const ChangeSetValue &change) { key_value_state[change.key] = change.value; },
...
S_TEST(E2E_Blockchain, Base) {
  ...
  TEST_TRY_STATUS(
      tester.expect_ok({BB::make_group_change({{2, 3, pk2.to_public_key()}}), BB::make_set_value(a, "hello a")}, pk));
...
S_TEST(E2E_Blockchain, Call) {
  ...
  TEST_TRY_STATUS(ct.update_call(0, {3, 4, 5}));
  TEST_TRY_STATUS(ct.check_shared_key());
  TEST_TRY_STATUS(ct.check_emoji_hash());
  return td::Status::OK();
}
...
TEST(Call, Basic_API) {
  ...
  auto zero_block =
      call_create_zero_block(key1, CallState{0, {CallParticipant{1, pkey1, 3}, CallParticipant{2, pkey2, 3}}}).value();

  auto call1 = call_create(key1, zero_block).value();
  auto call2 = call_create(key2, zero_block).value();

  auto block2 =
      call_create_change_state_block(call1, CallState{0, {CallParticipant{2, pkey2, 3}, CallParticipant{3, pkey3, 3}}})
          .value();
