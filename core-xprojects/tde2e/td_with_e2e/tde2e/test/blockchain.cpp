...
S_TEST(BlockchainValidation, ZeroBlock) {
  ...
  }
  {
    ...
    auto block = BB().with_height(0)
                     .with_block_hash({})
                     .with_group_state({{0, 3, alice_pk.to_public_key()}}, false, true)
  ...
  {
    TEST_DEBUG_VALUE(description, "Valid: zero block with other person in group state");
    auto block = BB().with_height(0)
                     .with_block_hash({})
  ...
  }
  {
    ...
    auto block = BB().with_height(0)
                     .with_block_hash({})
                     .with_group_state({{2, 3, bob_pk.to_public_key()}}, true, true)
                     .skip_shared_key_proof()
                     .build(alice_pk);
    TEST_TRY_STATUS(BT().expect_error(E::InvalidBlock_InvalidStateProof_Group, block));
  }
...
S_TEST(BlockchainValidation, GroupStateChanges) {
  ...
  // TODO(now): we should rething how permission works
  // 1. Who may set shared key?
  // 2. Who may rewrite group state with the **same** state?
  return td::Status::OK();
}
