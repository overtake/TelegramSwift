...
public struct IVReadState : Codable, Equatable {
    public let blockId:Int32
    public let blockOffset: Int32
...
public struct InstantViewAppearance: Codable, Equatable {
    ...
    public init(fontSerif: Bool, state: [MediaId: IVReadState]) {
    ...
    public func withUpdatedFontSerif(_ fontSerif: Bool) -> InstantViewAppearance {
        return InstantViewAppearance(fontSerif: fontSerif, state: self.state)
    }
    
    public func withUpdatedIVState(_ state: IVReadState, for mediaId:MediaId) -> InstantViewAppearance {
        var iv = self.state
        iv[mediaId] = state
...
public func updateInstantViewAppearanceSettingsInteractively(postbox: Postbox, _ f: @escaping (InstantViewAppearance) -> InstantViewAppearance) -> Signal<Void, NoError> {
...
public func ivAppearance(postbox: Postbox) -> Signal<InstantViewAppearance, NoError> {
