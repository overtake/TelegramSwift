...
NSDictionary *findCornerPoints(CGImage *bitmap) {
    CGImageRef imageRef = bitmap;
...
NSArray *binarizeAndFindCharacters(CGImage *inBmp, CGImage **outBinaryImage) {
    CGImageRef imageRef = inBmp;
    uint32_t width = (uint32_t)CGImageGetWidth(imageRef);
    uint32_t height = (uint32_t)CGImageGetHeight(imageRef);
...
NSString *performRecognition(CGImage *bitmap, int numRows, int numCols)
{
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"ocr_nn" ofType:@"bin"];
...
NSString *recognizeMRZ(CGImage *input, CGRect *outBoundingRect)
{
    CGImageRef binaryImage;
    ...
    for (NSArray *line in charRects)
    {
        ...
        for (NSValue *v in line)
        {
            ...
            CGImageRef charImage = CGImageCreateWithImageInRect(binaryImage, rect);
    ...
    CGImageRelease(binaryImage);
    CGImageRef charsImageRef = CGBitmapContextCreateImage(context);
    CGContextRelease(context);
    
    
    NSString *result = performRecognition(charsImageRef, (int)charRects.count, (int)[charRects.firstObject count]);
    if (result != nil && outBoundingRect != NULL)
    {
        CGRect firstRect = [[charRects.firstObject firstObject] CGRectValue];
        firstRect.origin.y = CGImageGetHeight(charsImageRef) - firstRect.origin.y;
        CGRect lastRect = [[charRects.lastObject lastObject] rectValue];
        lastRect.origin.y = CGImageGetHeight(charsImageRef) - lastRect.origin.y;
