...
private final class ChunkMediaPlayerContext {
    ...
    init(
        ...
        //playAndRecord: Bool, forceAudioToSpeaker: Bool, baseRate: Double, volume: Float, updatedRate: @escaping () -> Void, audioPaused: @escaping () -> Void
    ...
    fileprivate func setVolume(volume: Float) {
        self.volume = volume
        self.audioRenderer?.renderer.setVolume(volume)
    }
    ...
    private func tick() {
        ...
        if let bufferingProgress = bufferingProgress {
            playbackStatus = .buffering(initial: false, whilePlaying: isPlaying)
        } else if !rate.isZero {
        ...
        if self.lastStatusUpdateTimestamp == nil || self.lastStatusUpdateTimestamp! < statusTimestamp + 1.0 / 25.0 {
            ...
            let status = MediaPlayerStatus(generationTimestamp: CACurrentMediaTime(), duration: duration, dimensions: CGSize(), timestamp: statusTimestamp, baseRate: self.baseRate, volume: self.volume, seekId: self.seekId, status: playbackStatus)
...
public final class ChunkMediaPlayer {
    ...
    public func setVolume(volume: Float) {
        self.queue.async {
            if let context = self.contextRef?.takeUnretainedValue() {
                context.setVolume(volume: volume)
            }
        }
    }
    ...
    public func attachPlayerNode(_ node: MediaPlayerView) {
