...
void encodeARGBToARGB422A(uint8_t *argb422, uint8_t *a, uint8_t const *argb, int width, int height) {
    int i, j;
    int lineWidth = width * 2;
    for (j = 0; j < height; j++) {
        for (i = 0; i < width; i += 2) {
            int A1 = argb[(j * width + i) * 4 + 0];
            int R1 = argb[(j * width + i) * 4 + 1];
            int G1 = argb[(j * width + i) * 4 + 2];
            int B1 = argb[(j * width + i) * 4 + 3];
            
            int A2 = argb[(j * width + i) * 4 + 4];
            int R2 = argb[(j * width + i) * 4 + 5];
            int G2 = argb[(j * width + i) * 4 + 6];
            int B2 = argb[(j * width + i) * 4 + 7];
            
            argb422[j * lineWidth + (i / 2) * 4 + 0] = (uint8_t)((B1 + B2) >> 1);
            argb422[j * lineWidth + (i / 2) * 4 + 1] = G1;
            argb422[j * lineWidth + (i / 2) * 4 + 2] = (uint8_t)((R1 + R2) >> 1);
            argb422[j * lineWidth + (i / 2) * 4 + 3] = G2;
